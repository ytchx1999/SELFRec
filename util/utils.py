...
class MergeLayer(torch.nn.Module):
    def __init__(self, dim1, dim2, dim3, dim4, use_norm=True):
        super().__init__()
...
class RandEdgeSampler(object):
    def __init__(self, src_list, dst_list, ts_list, seed=None):
        self.edges = {}
    ...
    def sample_neg(self, src_list):
        ...
        if self.seed is None:
            ...
            for ind, u in enumerate(src_list):
                random_neg = self.random_state.choice(list(dst_set - self.edges[u]), 1)[0]
                neg_dst[ind] = random_neg
        return neg_dst
...
def get_neighbor_finder(data, uniform, max_node_idx=None):
    ...
    for source, destination, edge_idx, timestamp in zip(data.sources, data.destinations,
        ...
        ui_map_list[source].append(destination)
        ui_map_list[destination].append(source)

    return NeighborFinder(adj_list, ui_map_list, uniform=uniform)


class NeighborFinder:
    def __init__(self, adj_list, ui_map_list, uniform=False, seed=None):
        self.node_to_neighbors = []
        self.node_to_edge_idxs = []
        self.node_to_edge_timestamps = []

        self.ui_map_list = ui_map_list

        for neighbors in adj_list:
            # Neighbors is a list of tuples (neighbor, edge_idx, timestamp)
            # We sort the list based on timestamp
            sorted_neighhbors = sorted(neighbors, key=lambda x: x[2])  # 1/2
